...
describe("05 – Section Projets / Portfolio", () => {
  ...
  it("la section projets est présente sur la page", () => {
    cy.contains(/projets|projects|portfolio|réalisations|featured/i).should(
      "exist",
    );
  });
  ...
  it("chaque projet a un titre descriptif", () => {
    ...
    cy.get(
      'main h3:contains("Postman"), main h2:contains("Kids"), main h2:contains("Cineflix")',
    ).should("exist");
  });
  ...
  it("les liens GitHub / démo sont présents (En savoir plus, Démo)", () => {
    cy.get("main a").then(($links) => {
      const hasLink = $links.toArray().some((link) => {
  ...
  it("chaque projet affiche les technologies utilisées", () => {
    cy.get("main").within(() => {
      cy.contains(/react|node|typescript|postman|api|firebase|project/i).should(
        "exist",
      );
    });
  });
