...
describe('02 – Section Hero / Bannière', () => {
  ...
  it('affiche le nom complet "Chafik Mouram"', () => {
    cy.contains(/chafik\s+mouram/i).should('exist')
  })

  it('affiche le titre professionnel (QA / Testeur / Développeur)', () => {
    cy.contains(/testeur|QA|développeur|developer|automation/i).should('be.visible')
  })

  it('contient un call-to-action (bouton ou lien)', () => {
    cy.get('a[href*="#contact"], a[href*="#projects"], a[href*="projects"], button:visible')
      .should('have.length.gte', 1)
  })

  it('le CTA principal est cliquable', () => {
    cy.get('main a:visible, section:first-child a:visible, div[class*="hero"] a:visible, div[class*="banner"] a:visible')
      .first()
      .should('exist')
      .and('not.be.disabled')
  })
  ...
  it('la section hero est visible sans scroll (above the fold)', () => {
    cy.window().then((win) => {
      cy.contains(/chafik\s+mouram/i).then(($el) => {
  ...
  it('les liens dans le hero ouvrent les bonnes pages', () => {
    cy.get('header a[href], main a[href], section:first-child a[href]').each(($link) => {
